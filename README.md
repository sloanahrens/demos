...
```
cd demos
mkvirtualenv --no-site-packages demos
...
```
* * * * * ubuntu bash /home/ubuntu/apps/demos/repository/cron.sh fab $DEPLOYMENT_TARGET cron_jobs.test 
```
...
|--------|------|-----|-----|--------|--------|
|demos|tweet|`location`||||
|demos|facebook|`location`||||
|demos|email|`location`||||
|demos|new-comment||||
|demos|open-share-discuss||||
|demos|close-share-discuss||||
|demos|summary-copied||||
|demos|featured-tweet-action|`action`||``tweet_url``|
|demos|featured-facebook-action|`action`||``post_url``|
